...
void saveChild(const Relation& relation, const Range& range_with_ids, IdGenerator gen, DBStream stream)
{
	...
	const size_t total_sons = std::accumulate(range_with_ids.begin(), range_with_ids.end(), 0, [&relation](size_t accum, const typename Range::value_type& father) { 
		return accum + relation.m_accessor(father.second).size(); 
	});

	sons_refs.reserve(total_sons);
	for (const auto& father : range_with_ids) {
		const auto& father_sons = relation.m_accessor(father.second);
		std::transform(father_sons.begin(), father_sons.end(), std::back_inserter(sons_refs), [&](const child_type& son) {return value_type(son, father.first); });
	}
	save_impl(sons_refs, stream, gen);
}

template<class Range, class IdGenerator, class DBStream, class... Relations>
void saveChildren_impl(const std::tuple<Relations...>&, const Range&, IdGenerator, DBStream, TypedIndex<sizeof...(Relations)>) {}

template<size_t I, class Range, class IdGenerator, class DBStream, class... Relations>
void saveChildren_impl(const std::tuple<Relations...>& relations, const Range& range_with_ids, IdGenerator gen, DBStream stream, TypedIndex<I>)
{
	saveChild(std::get<I>(relations), range_with_ids, gen, stream);
	saveChildren_impl(relations, range_with_ids, gen, stream, TypedIndex<I+1>());
}
...
void saveChildren(const Range& range_with_ids, DBStream stream, IdGenerator gen) {
	...
	saveChildren_impl(OneToMany<undl_element_type >::relations(), range_with_ids, gen, stream, TypedIndex<0>());
}
template<class Range, class IdGenerator, class DBStream>
void save_impl(const Range& range, DBStream stream, IdGenerator gen) {
	...
	stream(zip(ids, range | make_transform([](const value_type& e) { return &e; })));
