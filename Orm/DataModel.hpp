...
template<class Accessor, class Writer>
Column<Accessor, Writer> createColumn(std::string fieldName, Accessor accessor, Writer writer, int length) {
	return Column<Accessor, Writer>(fieldName, accessor, writer, length);
}
template<class T, class FieldType>
auto createColumn(std::string fieldName, FieldType T::* field, int length) -> decltype(createColumn(fieldName, makeFieldAccessor(field), makeFieldAccessor(field), length))
{
	static_assert(!std::is_floating_point<FieldType>::value, "you have to specify number of decimals for floating type field. Use createNumberColumn instead");
	auto accessor = makeFieldAccessor(field);
	return createColumn(fieldName, accessor, accessor, length);
}

template<class Accessor, class Writer>
FloatingPointColumn<Accessor, Writer> createNumberColumn(std::string fieldName, Accessor accessor, Writer writer, int length, int decimals) 
{
	return FloatingPointColumn<Accessor, Writer>(fieldName, accessor, writer, length, decimals);
}
template<class T, class FieldType>
auto createNumberColumn(std::string fieldName, FieldType T::* field, int length, int decimals) -> decltype(createNumberColumn(fieldName, makeFieldAccessor(field), makeFieldAccessor(field), length, decimals))
{
	static_assert(std::is_floating_point<FieldType>::value, "use createColumn for non-floating fields");
	auto accessor = makeFieldAccessor(field);
